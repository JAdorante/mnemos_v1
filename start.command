#!/usr/bin/env bash
# Start Mnemos (macOS). Double-click in Finder, or: bash start.command
set -u
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$ROOT" || exit 1

if [ ! -x "$ROOT/.venv/bin/python" ]; then
    echo "Mnemos is not installed yet - run install.command first."
...
# call about "Mnemos.app is damaged".
xattr -dr com.apple.quarantine "$ROOT" 2>/dev/null

"$ROOT/.venv/bin/python" run_all.py
status=$?
[ "$status" -eq 0 ] || echo "Mnemos exited with status $status."
read -r -p "Press return to close." _
exit "$status"
