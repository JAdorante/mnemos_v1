...
# --- vinceo.ai memory bridge ---------------------------------------------------
# Give the browser agent read access to vinceo.ai's timeline so a task like
# "follow up on what Marc said about pricing" is grounded in what vinceo.ai actually
...
def make_memory_provider(limit=5, min_score=0.15):
    try:
        ...
        print(f"[bridge] vinceo.ai memory unavailable ({exc}); agent runs unaided.")
        return None
    ...
    def provider(goal: str) -> str:
        ...
        if not kept:
            return ""
        lines = ["RELEVANT MEMORIES FROM vinceo.ai (things you have already seen or "
