...
def _html_with_approval(page: str, *, next_url: str) -> HTMLResponse:
    ...
    return HTMLResponse(
        ...
        # These pages carry their own inline JS, so a stale copy silently runs
        # old client code against a new server. Versioned /static assets may be
        # cached; the SSR shell never is.
...
(function(){
  ...
  function place(){
    ...
    // unlock cards out of the feed. Otherwise register in the BR corner dock.
    if(!toast) return;
    try{
      const slot=document.getElementById('mnemosToastSlot');
      if(slotVisible(slot)){
        if(toast.parentNode!==slot) slot.appendChild(toast);
        toast.classList.add('mnemos-toast--docked');
        toast.style.position='relative';
        toast.style.right='auto';
        toast.style.bottom='auto';
        toast.style.left='auto';
        toast.style.zIndex='auto';
        toast.style.maxWidth='100%';
        toast.style.width='100%';
        toast.style.margin='0 0 12px';
        toast.style.boxSizing='border-box';
        return;
      }
      toast.classList.remove('mnemos-toast--docked');
      toast.style.maxWidth='340px';
      toast.style.width='auto';
      toast.style.margin='0';
      if(window.MnemosDock){
        MnemosDock.add(toast, MnemosDock.PRIORITY.toast);
      } else if(_toastHome&&toast.parentNode!==_toastHome){
        _toastHome.appendChild(toast);
      }
    }catch(e){}
  }
  window.MnemosPlaceToast=place;
  ...
  async function pollNudge(){
    ...
    try{
      ...
      if(d.first_win && d.first_win.href){
        ...
        if(ack) ack.onclick=async()=>{ await fetch('/first-run/nudge/ack',{method:'POST'}); toast.hidden=true; place(); };
        return;
      }
      if(d.unlock && d.unlock.show){
        ...
        if(u) u.onclick=async()=>{ await fetch('/first-run/unlock/ack',{method:'POST'}); toast.hidden=true; place(); };
      }
    }catch(e){}
  }
  pollNudge();
  setInterval(pollNudge, 20000);
  window.addEventListener('resize', function(){ if(toast&&!toast.hidden) place(); });
  ...
  async function updateBanner(){
    ...
    if(!b || d.state!=='update_available'){ bar.hidden=true; return; }
...
@router.post("/console/desktop-access/revoke")
def console_desktop_access_revoke(body: DesktopAccessRevoke) -> dict:
    """Remove a user-remembered app from the overlay (shipped apps unchanged)."""
    from desktop_agent import app_promotion
    result = app_promotion.revoke_promotion(body.app)
    if not result.get("ok"):
...
@router.post("/console/desktop-access/reload")
def console_desktop_access_reload() -> dict:
    """Re-read ~/.quill/apps.json into the live registry (Console hot reload)."""
    from desktop_agent import app_promotion, access
    app_promotion.reload_registry()
    return access.desktop_access_state()
...
def desktop_access_page() -> HTMLResponse:
    ...
    return _html_with_approval(DESKTOP_ACCESS_PAGE, next_url="/desktop-access")
...
async def chat_stream():
    """SSE push when chat events or agent state change (S-1).

    Clients call ``poll()`` on ``change``; 1–2s HTTP poll remains fallback.
    """
    import asyncio
    import json as _json
    ...
    def _sig() -> tuple:
        ...
        with w.lock:
            return (
                w.next_id,
                bool(w.awaiting or w.awaiting_fast),
                bool(w.busy or w.busy_fast),
                w.pending_todo is not None,
                w.error,
                w.url,
                round(w.cost, 4),
            )
    ...
    async def _events():
        last = None
        beats = 0
        for _ in range(7200):          # ~1h at 0.5s; client reconnects
            if _agent_disabled():
                yield "event: ping\ndata: {}\n\n"
            ...
            if last is not None and sig != last:
                last, beats = sig, 0
                yield ("event: change\ndata: "
                       + _json.dumps({"next": sig[0]}) + "\n\n")
            else:
                if last is None:
                    last = sig
                beats += 1
                if beats >= 30:         # ~15s heartbeat
                    beats = 0
                    yield "event: ping\ndata: {}\n\n"
...
def chat_ui() -> HTMLResponse:
    ...
    return _html_with_approval(CHAT_PAGE, next_url="/chat")
