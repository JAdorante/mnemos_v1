...
def _try(index, backend_name, backend_id, fourcc, out_dir):
    ...
    if not cap.isOpened():
        return f"  {backend_name:5} {fourcc or '(none)':6}  -> could not open"
    try:
        ...
        if not ok or frame is None:
            return f"  {backend_name:5} {fourcc or '(none)':6}  -> opened but no frame"
        h, w = frame.shape[:2]
        ...
        return (f"  {backend_name:5} {fourcc or '(none)':6}  -> {w}x{h}  "
