...
def test_core():
    ...
    model_router.router.complete_json = lambda *a, **k: {
        ...
        "items": [
            ...
            {"kind": "recommendation", "text": "Prepare a Sparrow status memo",
             "detail": "", "subject": "Sparrow", "confidence": 0.6, "source_fact_ids": [t1]},
    ...
    rec = items["Prepare a Sparrow status memo"]
    store.edit_reflection_item_text(rec["id"], "Prepare a Sparrow board memo")
    edited = store.get_reflection_item(rec["id"])
    check("edit updates text + marks 'edited'",
          edited["text"] == "Prepare a Sparrow board memo" and edited["review"] == "edited")
